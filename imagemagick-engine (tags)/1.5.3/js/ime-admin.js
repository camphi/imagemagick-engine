...
function imeStartResize()
{
    ...
    jQuery('#regenerate-images-metabox input').each(
        function () {
            var i = jQuery(this);
...
{
    jQuery.post(
        ajaxurl, { action: "ime_process_image", id: id, sizes: rt_sizes, force: rt_force }, function (data) {
...
function imeRegenMediaImage( id, sizes, force )
{
    ...
    jQuery.post(
        ajaxurl, { action: "ime_process_image", id: id, sizes: sizes, force: force }, function (data) {
            spinner.hide();
...
{
    jQuery("#ime-select-mode option").each(
        function (i,e) {
            var o = jQuery(this);
            var mode = o.val();
            if (o.is(':selected')) {
...
jQuery(document).ready(
    function ($) {
        ...
        $('#regenerate-images').click(
            function () {
                $('#regenerate-images-metabox img.ajax-feedback').css('visibility','visible');
                $.post(
                    ajaxurl, { action: "ime_regeneration_get_images" }, function (data) {
